      ...
      uint8_t *video_frame_delay_effective)
{
   ...
   if (     video_st->frame_count > refresh_rate
         ...
         && frame_delay_new
         && frame_delay_new <= video_st->frame_delay_target
         && frame_delay_new >= video_st->frame_time_reserve / 1000
   ...
   else if (abs(frame_time_dev) > 1000)
      frame_time_dev = 0;
   ...
   /* Shorten hold faster if there is plenty of leftover */
   if (     hold_count
   ...
   if (frame_time_over || frame_delay_new != frame_delay_cur)
      frame_time_dev = 0;
      ...
      settings_t *settings)
{
   ...
   if (settings->bools.video_frame_delay_auto)
   {
      ...
      if (video_st->frame_count >= 4 && !skip_delay)
      {
         ...
         video_frame_delay_leftover(video_st, runloop_st, refresh_rate, frame_time_interval,
