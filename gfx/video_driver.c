      ...
      bool *skip_delay)
{
   ...
   int16_t leftover              = 0;
   int16_t leftover_min          = 2000;
   uint8_t leftover_count        = video_st->frame_delay_leftover_count;
   uint8_t frame_delay_cur       = *video_frame_delay_effective;
   uint8_t frame_delay_new       = *video_frame_delay_effective;
   bool frame_time_over          = false;
   bool frame_time_near          = false;

   /* Calibration levels */
   frame_time_index             -= (frame_time_index) ? 1 : 0;
   frame_time                    = video_st->frame_time_samples[frame_time_index];
   frame_time_over               = frame_time > frame_time_target * 1.85f;
   frame_time_near               = abs(frame_time - frame_time_target) < frame_time_target * 0.15f;

   /* No delay when frame and core times are tight */
   if (frame_time_over || runloop_st->core_run_time >= frame_time_target)
   {
      *skip_delay = true;
      video_st->frame_time_samples[frame_time_index] = 0;
   }

   /* New delay */
   frame_delay_new = (frame_time_target - runloop_st->core_run_time - leftover_min) / 1000.0f;

   /* Use average when increasing */
   if (frame_delay_new > frame_delay_cur)
      frame_delay_new = (frame_delay_new + frame_delay_cur) / 2;

   /* Never go above set target */
   ...
   /* Increase always in small steps */
   if (frame_delay_new > frame_delay_cur)
   {
      if (!leftover_count)
      {
         frame_delay_new = frame_delay_cur + 1;
         leftover_count++;
      }
   ...
   /* Remaining time for presentation, shaders etc */
   leftover = frame_time_target - (frame_delay_new * 1000) - runloop_st->core_run_time;

   if (leftover < leftover_min)
   {
      int8_t diff = ceil((leftover - leftover_min) / 1000.0f);

      if (diff < 0)
         frame_delay_new += diff;

      leftover_count = refresh_rate * 2;
   }

   if (leftover_count > refresh_rate * 4)
      leftover_count = 0;

   *video_frame_delay_effective         = frame_delay_new;
   video_st->frame_delay_leftover_count = leftover_count;

#if FRAME_DELAY_AUTO_DEBUG
   RARCH_DBG("[Video]: Frame delay frametime:%5d near:%d over:%d core:%5d leftover:%5d eff:%2d lc:%2d.\n",
         frame_time, frame_time_near, frame_time_over, runloop_st->core_run_time, leftover, *video_frame_delay_effective, leftover_count);
#endif
}
      ...
      settings_t *settings)
{
   ...
   if (settings->bools.video_frame_delay_auto)
   {
      ...
      {
         prevent_plus = false;
         skip_update = frame_time_interval * 8;
      ...
      if (video_st->frame_delay_target != video_frame_delay)
      {
         ...
         RARCH_DBG("[Video]: Frame delay reset to %d ms.\n", video_frame_delay);
      }

      /* Immediate quick correction and recovery */
      if (video_st->frame_count > 1 && !skip_delay)
         video_frame_delay_leftover(video_st, runloop_st, refresh_rate, &video_frame_delay_maybe, &skip_delay);

      if (video_frame_delay_maybe > video_frame_delay)
         video_frame_delay_maybe = video_frame_delay;

      if (video_frame_delay_maybe != video_frame_delay_effective)
      {
         if (!prevent_plus || video_frame_delay_maybe < video_frame_delay_effective)
         {
            if (!skip_delay)
            {
               frame_time_update = false;
      ...
      if (video_frame_delay_effective > 0 && frame_time_update)
      {
         ...
         if (vfda.delay_decrease > 0)
         {
            ...
            RARCH_DBG("[Video]: Frame delay limit by %d ms to %d ms due to frame time average: %d > %d.\n",
...
void video_frame_delay_auto(video_driver_state_t *video_st, video_frame_delay_auto_t *vfda)
{
   ...
   if (frame_time_avg > frame_time_target && frame_time_delta > frame_time_target * 0.05f)
      video_st->frame_time_count_pos_avg++;
   else
      video_st->frame_time_count_pos_avg = 0;
         ...
         || (frame_time_delta > frame_time_limit_max))
   {
      ...
      /* Over half of interval frames are above minimum level */
         ...
         mode = 2;
      /* Scanline sync temporal finetune */
      else if (video_st->frame_time_count_pos_avg >= frame_time_frames)
         mode = 3;
   ...
   if (frame_time_avg > frame_time_limit_min || mode > 0)
   {
      ...
      if (     (frame_time_avg > frame_time_limit_med || mode == 1 || mode == 2)
